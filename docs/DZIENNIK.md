...
| 26 | awarie hosta przy dołączaniu klienta bierze się z rozgłoszenia do **wiszącego** słuchacza (`0x1419E7BA1`), którego gra sprawdza tylko na null | z markerem `fix_lista` host przeżywa dołączenie; w logu `LISTA: pominietych wiszacych sluchaczy: N`, `N ≥ 1` | jeden przebieg z dołączeniem klienta; próba kontrolna = zdjęcie markera | — |
