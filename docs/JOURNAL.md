...
|---|---|---|---|---|
| 38 | **wejście klienta ginie w `UPlayerInput::InputKey` (`0x141A01390`) albo w przetwarzaniu wiązań** — siedem ogniw wyżej jest już zmierzonych i wszystkie są zdrowe, łącznie z `PlayerInput`, który NIE jest nullem (`KNOWLEDGE.md` §3m) | klient i host wykonują ten sam kod (ta sama tablica metod `0x1450A3470`), więc różnica siedzi w DANYCH: w tablicy wiązań albo w stanie klawiszy | **najpierw dekompilacja `0x141A01390`, bez gry** — dopiero jej pseudokod powie, co porównywać. Potem liczniki i porównanie danych wiązań host kontra klient | — |
...
**Kolejność: 38 → 39 → 32 → 29 → 30.** Bez wejścia u klienta co-op jest
niegrywalny, więc 38 jest pierwsze, a 39 zaraz po nim (to ta sama robota na
sąsiednim rozdzielaczu). 32 jest dalej, bo host pada po ~50 min i psuje każdy
