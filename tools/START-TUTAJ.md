...
## WZORZEC ZDROWEJ GRY — po co gracz gra w czystą grę

**Gracz gra w niemodowaną grę CELOWO, żeby zostawić zapas zdrowych logów do
porównań przy testach multi.** Powiedział to wprost 12.08. To nie jest granie
„przy okazji" — to zbieranie materiału odniesienia i tak należy je traktować.
Gdy mówi, że zagra dłużej, **nie zakładać, że chodzi o sesję co-op**.

Dlaczego to jest warte całej sesji: bez wzorca nie wiadomo, **która wartość jest
dziwna**. Świeży przykład kosztu — liczbę `522,8` odczytano jako
`615 × 0,83` (mnożnik staminy) i zbudowano na tym hipotezę 21. Naprawdę to
`615 × 0,85`, czyli chód w bok. Wzorzec rozstrzygnąłby to w pierwszej minucie.

Zbieranie jest **bierne i automatyczne**:

```
setsid nohup tools/obserwator.sh > /dev/null 2>&1 < /dev/null &
```

Obserwator wykrywa każde uruchomienie gry — czystej i modowanej — i zapisuje
sesję do `logs/sesje/<data>-<rodzaj>/`. Gracz nie musi niczego uruchamiać ani
uprzedzać; gdy powie „tu się zepsuło", materiał z tamtej chwili już jest.

Odczyt po fakcie:

```
tools/rejestrator.py --podsumuj logs/sesje/<sesja>/szereg.tsv
```

zwija godziny do rozkładów: ile procent czasu który tryb ruchu, który stan
maszyny, które warunki były prawdziwe, w jakim zakresie chodziły atrybuty.
**Porównanie „czysta kontra klient" jest jedynym wiarygodnym sposobem
powiedzenia, która wartość jest zepsuta.**

Pierwszy fragment wzorca (12.08, host solo, 24 s) już rozstrzyga rzeczy, których
inaczej nie dałoby się orzec:

| | zdrowy gracz | pionek klienta (11.08) |
|---|---|---|
| tryb ruchu | `Walking` 78%, `Falling` 22% | **`Falling` 100%** |
| stan maszyny | 0 / 4 / 1 | **0 zawsze** |
| `IsOnGround` | 78% czasu | **0%** |
| magazynek | `ClipSize 6`, w klipie `6`, `PendingClipRefill 0` | w klipie `0` |

Do tego jedna rzecz, która wprost zapobiegła fałszywemu alarmowi: **`Stamina`
u zdrowego gracza schodzi do `-26`** (mechanika `StaminaOverdraw`). Wartość
ujemna u klienta zostałaby wzięta za usterkę.

Uwaga na pułapkę, która już raz o mało nie zatruła wzorca: część atrybutów to
`FGameplayAttributeData` (8 B, `BaseValue` + `CurrentValue`), a część **zwykłe
floaty**. Rejestrator bierze przesunięcie z TYPU właściwości; gdyby dokładał
`+4` wszystkim, etykiety kolumn przesunęłyby się o jedno pole, a wartości dalej
wyglądałyby wiarygodnie. Wyłapało to dopiero porównanie ze `stan-gracza.py`.
